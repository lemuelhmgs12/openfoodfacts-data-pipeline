...
def write_batch(settings, products, ingest_date, count, run_time, s3_client=None):

    s3 = s3_client if s3_client is not None else boto3.client("s3")

    s3_key = f"{settings.raw_prefix}/ingest_date={ingest_date.isoformat()}/batch_{run_time}_{count:03d}.json"

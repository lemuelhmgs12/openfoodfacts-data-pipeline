...
def test_write_batch_to_s3(fake_s3, fake_products, settings):
    ...
    result = write_batch(settings, fake_products, ingest_date, count, s3_client=fake_s3)
    stored = json.loads(fake_s3.storage[result])


    assert result == "raw/openfoodfacts/ingest_date=2026-09-05/batch_000.json"
...
def test_write_batch_raises_on_s3_failure(fake_s3, fake_products, settings, mocker):
    ...
    with pytest.raises(ClientError):
        write_batch(settings, fake_products, date(2026, 9, 5), 0, s3_client=fake_s3)